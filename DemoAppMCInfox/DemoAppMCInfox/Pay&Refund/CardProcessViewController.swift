...
class CardProcessViewController: UIViewController , PinOnGlass_Delegate{
    ...
    func startTransactionProcess() {
        ...
        if(isRefund) {
              payment.startRefund(slipNumber: self.slipNo, dAmount: Double(self.amount ) ?? 0.0, sSeqNumber: "1234567", sProductCategoryCode: "1234567", sDeviceName: "IDTECH3171") // idtech3187
        }
        else {
        payment.startTransaction(dAmount: Double(amount ?? "0.00") as! Double, sSeqNumber: "", sProductCategoryCode: "001", sDeviceName: "IDTECH3171") // IDTECH3187
        }
